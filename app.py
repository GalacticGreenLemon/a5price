...
def draw_label(draw, img, data, offset_x):
    ...
    if data['StatusText']:
        ...
        line_h = draw.textbbox((0, 0), "Ag", font=font_status)[3] + 0
        ...
        for word in words:
            ...
            if draw.textlength(test, font=font_status) <= max_w:
                current.append(word)
            else:
                if current:
...
def build_download_all_html(files):
    """
    A single button that triggers a separate browser download for every
    label image, without bundling them into a zip. Browsers can throttle
    many downloads fired at once, so each one is staggered slightly —
    unnoticeable for the handful of labels this app generates at a time.
    """
    items_js = ",".join(
        '{name:"%s", data:"data:image/png;base64,%s"}'
        % (filename.replace('"', ""), base64.b64encode(data).decode())
        for filename, data in files
    )
    return f"""
    <div>
        <button id="dl-all-btn"
                style="padding:10px 18px; font-size:15px; cursor:pointer;
                       background:#ff4b4b; color:white; border:none; border-radius:6px;">
            ⬇️ Descarcă toate imaginile ({len(files)})
        </button>
    </div>
    <script>
        const dlFiles = [{items_js}];
            ...
            if st.button("Generează etichetele", type="primary", key="generate_csv"):
                ...
                if want_zip_csv:
                    ...
                    if len(files) == 1:
                        ...
                        components.html(build_download_all_html(files), height=70)
